...
# 加载分割模型
seg_model = torch.load('models/unet_iter_1300000.pt', map_location='cpu') # , map_location='cpu'
...
if ishard:
    checkpoint = torch.load('models/hard_retrive_ckpt.pt', map_location='cpu')
else:
    checkpoint = torch.load('models/retrive_ckpt.pt', map_location='cpu')
pre_model.load_state_dict({k.replace('module.', ''): v for k, v in dict(checkpoint).items()})
...
def execute_retrival(brand,img,seg_model,search_model):
    ...
    for pt, mask in enumerate(masks):
        ...
        # predict
        out_pred, out_clf = search_model(sub_img)
        # predict clf
        ...
        for index, count in enumerate(labels_count):
            ...
            start += count
        out_clf = out_clf.data.cpu().numpy()
        # 得到类名
